...
public:
  explicit ApproxConv2DRefOp(OpKernelConstruction *ctx)
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    OP_REQUIRES(ctx, inDepth == filter.dim_size(2),
                errors::InvalidArgument(
                    "input and filter must have same depth: ", inDepth, " vs ",
                    filter.dim_size(2)));
    ...
    OP_REQUIRES(ctx,
                FastBoundsCheck(inputRowsRaw, std::numeric_limits<int>::max()),
    ...
    OP_REQUIRES(ctx,
                FastBoundsCheck(inputColsRaw, std::numeric_limits<int>::max()),
    ...
    OP_REQUIRES_OK(ctx,
                   GetWindowedOutputSize(inputRows, filterRows, 1, strideRows,
                                         m_padding, &outRows, &padRows));
    OP_REQUIRES_OK(ctx,
                   GetWindowedOutputSize(inputCols, filterCols, 1, strideCols,
                                         m_padding, &outCols, &padCols));

    TensorShape outShape =
        ShapeFromFormat(m_dataFormat, batch, outRows, outCols, outDepth);

    Tensor *output = nullptr;
    OP_REQUIRES_OK(ctx, ctx->allocate_output(0, outShape, &output));

    VLOG(2) << "ApproxConv2D: inDepth = " << inDepth
            << ", inputCols = " << inputCols << ", filterCols = " << filterCols
            << ", inputRows = " << inputRows << ", filterRows = " << filterRows
            << ", strideRows = " << strideRows
            << ", strideCols = " << strideCols << ", outDepth = " << outDepth;
    ...
    convFunctor(ctx, m_approxOp, input.flat<T>().data(), batch, inputRows,
                inputCols, inDepth, filter.flat<T>().data(), filterRows,
                filterCols, outDepth, strideRows, strideCols, m_padding,
...
public:
  void operator()(OpKernelContext *ctx,
                  ...
                  const T *inputData, int inputBatches, int inputHeight,
                  int inputWidth, int inputDepth, const T *filterData,
...
public:
  void operator()(OpKernelContext *ctx,
                  const NullApproxOpType_t<CPUDevice, T, AT> &approxOp,
                  const T *inputData, int inputBatches, int inputHeight,
                  int inputWidth, int inputDepth, const T *filterData,
                  int filterHeight, int filterWidth, int filterCount,
                  int strideRows, int strideCols, Padding padding,
    ...
    if (padding == VALID) {
      filterLeftOffset =
          ((outputWidth - 1) * strideCols + filterWidth - inputWidth + 1) / 2;
      filterTopOffset =
          ((outputHeight - 1) * strideRows + filterHeight - inputHeight + 1) /
          2;
    } else {
      filterLeftOffset =
          ((outputWidth - 1) * strideCols + filterWidth - inputWidth) / 2;
      filterTopOffset =
          ((outputHeight - 1) * strideRows + filterHeight - inputHeight) / 2;
    }
      ...
      for (int outY = 0; outY < outputHeight; ++outY) {
        for (int outX = 0; outX < outputWidth; ++outX) {
          ...
          for (int outChannel = 0; outChannel < filterCount; ++outChannel) {
            ...
            // Multiply filter kernel with patch of input image around its
            // current position, summing the results into current channel of the
            // output pixel.
              ...
              for (int filterX = 0; filterX < filterWidth; ++filterX) {
                ...
                for (int inChannel = 0; inChannel < inputDepth; ++inChannel) {
                  ...
                  if ((inX >= 0) && (inX < inputWidth) && (inY >= 0) &&
                      (inY < inputHeight)) {
                    inputValue = inputData[(batch * inputHeight * inputWidth *
                                            inputDepth) +
                                           (inY * inputWidth * inputDepth) +
                                           (inX * inputDepth) + inChannel];
                  } else {
                    inputValue = T(0);
                  }

                  const T filterValue =
                      filterData[(filterY * filterWidth * inputDepth *
                                  filterCount) +
                                 (filterX * inputDepth * filterCount) +
                                 (inChannel * filterCount) + outChannel];
                  total += (inputValue * filterValue);
                }
              }
            }

            outputData[(batch * outputHeight * outputWidth * filterCount) +
                       (outY * outputWidth * filterCount) +
                       (outX * filterCount) + outChannel] = total;
          }
        }
      }
    }
...
public:
  void operator()(OpKernelContext *ctx,
                  const TableApproxOpType_t<CPUDevice, T, AT> &approxOp,
                  const T *inputData, int inputBatches, int inputHeight,
                  int inputWidth, int inputDepth, const T *filterData,
                  int filterHeight, int filterWidth, int filterCount,
                  int strideRows, int strideCols, Padding padding,
        ...
        for (int outX = 0; outX < outputWidth; ++outX) {
          ...
          for (int outChannel = 0; outChannel < filterCount; ++outChannel) {
            ...
            for (int filterY = 0; filterY < filterHeight; ++filterY) {
              for (int filterX = 0; filterX < filterWidth; ++filterX) {
                ...
                for (int inChannel = 0; inChannel < inputDepth; ++inChannel) {
                  ...
                  filterPatchTotal +=
                      filterValue; // TODO: This can be factored out as it
                                   // should be constant accross all patches

                  // Quantize values and use lookup table to compute the product
                  const uint32 inputValueQ =
                      AT(((inputValue - approxOp.quantProps.input.offset) *
                          approxOp.quantProps.input.invScale) +
                         T(0.5));
                  const uint32 filterValueQ =
                      AT(((filterValue - approxOp.quantProps.filter.offset) *
                          approxOp.quantProps.filter.invScale) +
                         T(0.5));
                  const uint32 prodValueQ =
                      approxOp.lookupTable[(inputValueQ << approxOp.bitWidth) |
                                           filterValueQ];
            ...
            total = total * approxOp.quantProps.s1xS2 +
                    ...
                    T(filterHeight * filterWidth * inputDepth) *
                        approxOp.quantProps.m1xM2;
            outputData[(batch * outputHeight * outputWidth * filterCount) +
                       (outY * outputWidth * filterCount) +
                       (outX * filterCount) + outChannel] = total;
          }
...
#define REGISTER_CPU(T)                                                        \
  REGISTER_KERNEL_BUILDER(                                                     \
      ...
      ApproxConv2DRefOp<                                                       \
          T, uint8, NullApproxOpType_t,                                        \
          ReferenceApproxConvFunctor<T, uint8, NullApproxOpType_t>>);
...
#define REGISTER_CPU(T)                                                        \
  REGISTER_KERNEL_BUILDER(                                                     \
      ...
      ApproxConv2DRefOp<                                                       \
          T, uint8, TableApproxOpType_t,                                       \
          ReferenceApproxConvFunctor<T, uint8, TableApproxOpType_t>>);
