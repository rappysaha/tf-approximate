...
public:
  void operator()(OpKernelContext *ctx, const T1 *inputData, int inputBatches,
                  int inputHeight, int inputWidth, int inputDepth,
                  const T2 *filterData, int filterHeight, int filterWidth,
                  int filterCount, int strideRows, int strideCols,
                  Padding padding, T3 *outputData, int outputHeight,
                  int outputWidth) {
    if ((inputBatches <= 0) || (inputWidth <= 0) || (inputHeight <= 0) ||
        (inputDepth <= 0)) {
      LOG(WARNING) << "ApproxConv2D was called with bad input dimensions: "
                   << inputBatches << ", " << inputHeight << ", " << inputWidth
                   << ", " << inputDepth;
      return;
    }
    ...
    if ((outputWidth <= 0) || (outputHeight <= 0)) {
      LOG(WARNING)
          << "ApproxConv2D was called with bad output width or height: "
    ...
    if (filterHeight == 1 && filterWidth == 1 && strideRows == 1 &&
      ...
      gemmFunctor(ctx, m, n, k, inputData, lda, filterData, ldb, outputData,
                  ldc);
      return;
    ...
    OP_REQUIRES(
        ctx, (filterValueCount * sizeof(T1)) <= kMaxChunkSize,
        errors::InvalidArgument("Im2ColApprox patch too large for buffer"));
    const int64 patchesPerChunk =
        kMaxChunkSize / (filterValueCount * sizeof(T1));
    const int64 chunkValueCount =
    ...
    OP_REQUIRES_OK(ctx,
                   ctx->allocate_temp(DataType::DT_FLOAT, im2ColTmpBufferShape,
                                      &im2ColTmpBuffer));

    T1 *pIm2ColBufferData = im2ColTmpBuffer.flat<T1>().data();

    const int64 patchCount = (inputBatches * outputHeight * outputWidth);
    const int64 chunkCount =
        (patchCount + (patchesPerChunk - 1)) / patchesPerChunk;

    for (int64 chunkIndex = 0; chunkIndex < chunkCount; ++chunkIndex) {
      const int64 patchIndexStart = chunkIndex * patchesPerChunk;
      const int64 patchIndexEnd =
          std::min(patchIndexStart + patchesPerChunk, patchCount);

      for (int64 patchIndex = patchIndexStart; patchIndex < patchIndexEnd;
           ++patchIndex) {
        ...
        const T1 *inputBatchStart =
            inputData + (batch * inputHeight * inputWidth * inputDepth);
        ...
        T1 *im2colPatchStart =
            pIm2ColBufferData + (patchIndexWithinChunk * filterValueCount);

        for (int filterY = 0; filterY < filterHeight; ++filterY) {
          const int inY = inYOrigin + filterY;
          T1 *im2ColRowStart =
              im2colPatchStart + (filterY * filterWidth * inputDepth);
          ...
          if ((inY < 0) || (inY >= inputHeight)) {
            ...
            const int centerCopyCount =
                filterWidth - (leftZeroCount + rightZeroCount);

            if (leftZeroCount > 0) {
              T1 *im2ColLeftStart = im2ColRowStart;
              T1 *im2ColLeftEnd =
                  im2ColLeftStart + (leftZeroCount * inputDepth);

              std::fill(im2ColLeftStart, im2ColLeftEnd, T1(0));
            }

            if (centerCopyCount > 0) {
              const T1 *inputRowStart = inputBatchStart +
                                        (inY * inputWidth * inputDepth) +
                                        (std::max(0, inXOrigin) * inputDepth);
              const T1 *inputRowEnd =
                  inputRowStart + (centerCopyCount * inputDepth);
              T1 *im2ColCenterStart =
                  im2ColRowStart + (leftZeroCount * inputDepth);
            ...
            if (rightZeroCount > 0) {
              ...
              T1 *im2ColRightEnd =
                  im2ColRightStart + (rightZeroCount * inputDepth);
...
public:
  explicit ApproxConv2DUsingGemmOp(OpKernelConstruction *ctx)
      : BinaryOp<T>(ctx) {
...
#define REGISTER_CPU(T)                                                        \
  REGISTER_KERNEL_BUILDER(                                                     \
      ...
      ApproxConv2DUsingGemmOp<                                                 \
          T,                                                                   \
          Im2ColApproxConvFunctor<T, T, T, ReferenceGemmFunctor<T, T, T>>>);
