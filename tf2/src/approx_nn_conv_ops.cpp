 ...
 */
template <typename T> struct TempBufferType_t;

template <> struct TempBufferType_t<Eigen::half> {
  static DataType Get() { return DataType::DT_HALF; }
};

template <> struct TempBufferType_t<float> {
  static DataType Get() { return DataType::DT_FLOAT; }
};

template <> struct TempBufferType_t<double> {
  static DataType Get() { return DataType::DT_DOUBLE; }
};

template <> struct TempBufferType_t<uint8> {
  static DataType Get() { return DataType::DT_UINT8; }
};
...
template <typename Device, typename T, typename AT,
          template <typename, typename, typename> class ApproxOpType>
struct LaunchGeneric {
  void operator()(OpKernelContext *ctx, const Tensor &input,
                  const Tensor &filter, int rowPadding, int colPadding,
                  int rowStride, int colStride, int rowDilation,
                  int colDilation, const Padding &padding,
                  const std::vector<int64> &explicitPaddings, Tensor *output,
                  TensorFormat dataFormat,
                  const ApproxOpType<Device, T, AT> &approxOp) {
    ...
    if (filter.dim_size(0) == 1 && filter.dim_size(1) == 1 && rowStride == 1 &&
      ...
      const int m =
          int(input.dim_size(0) * input.dim_size(1) * input.dim_size(2));
      ...
      ApproxConvGEMMKernelCombined<Device, T, AT, ApproxOpType>()(
          ctx->eigen_device<Device>(), approxOp, m, n, k, T(0),
          input.flat<T>().data(), lda, filter.flat<T>().data(), ldb, T(0),
          output->flat<T>().data(), ldc);
      ...
      const int n = int(filter.dim_size(3));
      const int k =
          int(input.dim_size(1) * input.dim_size(2) * input.dim_size(3));
      const int lda = k;
      const int ldb = int(filter.dim_size(3));
      const int ldc = int(filter.dim_size(3));
      ...
      OP_REQUIRES(ctx, (patchLength) <= kMaxChunkSize,
                  errors::InvalidArgument(
                      "ApproxConv2DOp: Patch too large for buffer"));

      int64 totalPatchesCount =
...

      {
        ...
        OP_REQUIRES_OK(ctx, ctx->allocate_temp(TempBufferType_t<AT>::Get(),
                                               im2ColTmpBufferShape,
                                               &im2ColTmpBuffer));
      }
...

      {
        ...
        OP_REQUIRES_OK(ctx, ctx->allocate_temp(TempBufferType_t<T>::Get(),
                                               shape, &patchCoeffTmpBuffer));
      }
      ...
      ApproxFilterCorrCoeff<Device, T, AT, ApproxOpType>()(
          ctx->eigen_device<Device>(), approxOp, filter.tensor<T, 4>(),
          filterCoeffTmpBuffer.flat<T>());
      /*for(unsigned i = 0; i < filterCoeffTmpBuffer.shape().num_elements();
         ++i) std::cout << pFilterCoeffData[i] << std::endl;*/

      if (padding == EXPLICIT) {
        ctx->SetStatus(
            errors::Unimplemented("Explicit padding is not implemented."));
      } else {
        ...
        for (int64 i = 0; i < totalPatchesCount; i += patchesPerChunk) {
          ...
          int patchOffset =
              int(i % (output->dim_size(1) * output->dim_size(2)));
          int patchesCount =
              int(std::min(patchesPerChunk, totalPatchesCount - i));

          const T *pInputData =
              input.flat<T>().data() +
              batch * input.dim_size(1) * input.dim_size(2) * input.dim_size(3);

          ApproxConvIm2ColKernel<Device, T, AT, ApproxOpType>()(
              ctx->eigen_device<Device>(), approxOp, pInputData,
              input.dim_size(3), input.dim_size(2), input.dim_size(1),
              output->dim_size(2), output->dim_size(1), filter.dim_size(1),
              filter.dim_size(0), colPadding, rowPadding, colStride, rowStride,
              colDilation, rowDilation, patchOffset, patchesCount,
              pIm2ColBufferData, pPatchCoeffData);
          ...
          ApproxConvGEMMKernel<Device, T, AT, ApproxOpType>()(
              ctx->eigen_device<Device>(), approxOp, m, n, k, T(0),
              pIm2ColBufferData, lda, filter.flat<T>().data(), ldb,
              pPatchCoeffData, pFilterCoeffData, T(0), pOutputData, ldc);
...
struct LaunchApproxConv2DOp<CPUDevice, T, AT, ApproxOpType> {
  void operator()(OpKernelContext *ctx, const Tensor &input,
                  const Tensor &filter, int rowPadding, int colPadding,
                  int rowDilation, int colDilation, int rowStride,
                  int colStride, const Padding &padding,
                  const std::vector<int64> &explicitPaddings, Tensor *output,
                  TensorFormat dataFormat,
                  const ApproxOpType<CPUDevice, T, AT> &approxOp) {
    ...
    LaunchGeneric<CPUDevice, T, AT, ApproxOpType>()(
        ctx, input, filter, rowPadding, colPadding, rowStride, colStride,
        rowDilation, colDilation, padding, explicitPaddings, output, dataFormat,
        approxOp);
  }
};
...
Status tensorflow::InitApproxConv2DParameters(const OpKernelConstruction *ctx,
  ...
  if (ctx->HasAttr("explicit_paddings")) {
    TF_RETURN_IF_ERROR(
        ctx->GetAttr("explicit_paddings", &params->explicitPaddings));
  }
  ...
  TF_RETURN_IF_ERROR(CheckValidPadding(params->padding,
                                       params->explicitPaddings,
                                       /*num_dims=*/4, dataFormat));

  return absl::OkStatus();
}

Status tensorflow::ComputeApproxConv2DDimensions(
    const ApproxConv2DParameters &params, const Tensor &input,
    const Tensor &filter, ApproxConv2DDimensions *dimensions) {
  ...
  const int dilationRows =
      GetTensorDim(params.dilations, params.dataFormat, 'H');
  const int dilationCols =
      GetTensorDim(params.dilations, params.dataFormat, 'W');
  ...
  TF_RETURN_IF_ERROR(GetWindowedOutputSizeVerbose(
      inputRows, filterRows, dilationRows, strideRows, params.padding, &outRows,
      &padRowsBefore, &padRowsAfter));
  TF_RETURN_IF_ERROR(GetWindowedOutputSizeVerbose(
      inputCols, filterCols, dilationCols, strideCols, params.padding, &outCols,
      &padColsBefore, &padColsAfter));
...
public:
  explicit ApproxConv2DOp(OpKernelConstruction *ctx)
      : OpKernel(ctx), m_approxOp(ctx) {
  ...
  void Compute(OpKernelContext *ctx) override {
    ...
    // std::cout << "PADDING: " << dimensions.padRowsBefore << " " <<
    // dimensions.padColsBefore << std::endl;
    ...
    m_launcher(
        ctx, input, filter, dimensions.padRowsBefore, dimensions.padColsBefore,
        dimensions.dilationRows, dimensions.dilationCols, dimensions.strideRows,
        dimensions.strideCols, m_params.padding, m_params.explicitPaddings,
        output, m_params.dataFormat, m_approxOp);
  }
...
template struct LaunchApproxConv2DOp<CPUDevice, Eigen::half, Eigen::half,
                                     NullApproxOpType_t>;
template struct LaunchApproxConv2DOp<CPUDevice, float, float,
                                     NullApproxOpType_t>;
template struct LaunchApproxConv2DOp<CPUDevice, double, double,
                                     NullApproxOpType_t>;
} // namespace tensorflow
...
namespace tensorflow {
template struct LaunchApproxConv2DOp<CPUDevice, float, uint8,
                                     TableApproxOpType_t>;
}
...
template <typename T, typename AT,
          template <typename, typename, typename> class ApproxOpType>
void LaunchApproxConv2DOp<GPUDevice, T, AT, ApproxOpType>::operator()(
    OpKernelContext *ctx, const Tensor &inputParam, const Tensor &filter,
    int rowPadding, int colPadding, int rowDilation, int colDilation,
    int rowStride, int colStride, const Padding &padding,
    const std::vector<int64> &explicitPaddings, Tensor *output,
  ...
  if (patchRows == 1 && patchCols == 1 && !isGroupedConvolution &&
    ...
    /*auto pA = AsDeviceMemory(input.template flat<T>().data(), input.template
    flat<T>().size()); auto pB = AsDeviceMemory(filter.template
    flat<T>().data(), filter.template flat<T>().size()); auto pC =
    AsDeviceMemory(output->template flat<T>().data(), output->template
    flat<T>().size());*/

    ApproxConvGEMMKernelCombined<GPUDevice, T, AT, ApproxOpType>()(
        ctx->eigen_device<GPUDevice>(), approxOp, m, n, k, T(0),
        input.flat<T>().data(), k, filter.flat<T>().data(), n, T(0),
        output->flat<T>().data(), n);

    return;
  } else if (patchRows == inRows && patchCols == inCols &&
             !isGroupedConvolution && rowDilation == 1 && colDilation == 1 &&
             padding == VALID && dataFormat == FORMAT_NHWC) {
...

    {
      ...
      OP_REQUIRES_OK(ctx, ctx->allocate_temp(TempBufferType_t<AT>::Get(), shape,
                                             &im2ColTmpBuffer));
    }
...

    {
      TensorShape shape;
      TensorShapeUtils::MakeShape(&patchesPerChunk, 1, &shape);
      OP_REQUIRES_OK(ctx, ctx->allocate_temp(TempBufferType_t<T>::Get(), shape,
                                             &patchCoeffTmpBuffer));
    }
    ...
    ApproxFilterCorrCoeff<GPUDevice, T, AT, ApproxOpType>()(
        ctx->eigen_device<GPUDevice>(), approxOp, filter.tensor<T, 4>(),
        filterCoeffTmpBuffer.flat<T>());
    ...
    if (padding == EXPLICIT) {
      ...
      for (int64 i = 0; i < totalPatchesCount; i += patchesPerChunk) {
        ...
        ApproxConvIm2ColKernel<GPUDevice, T, AT, ApproxOpType>()(
            ctx->eigen_device<GPUDevice>(), approxOp, pInputData,
            input.dim_size(3), input.dim_size(2), input.dim_size(1),
            output->dim_size(2), output->dim_size(1), filter.dim_size(1),
            filter.dim_size(0), colPadding, rowPadding, colStride, rowStride,
            colDilation, rowDilation, patchOffset, patchesCount,
...
namespace tensorflow {
template struct LaunchApproxConv2DOp<GPUDevice, Eigen::half, Eigen::half,
                                     NullApproxOpType_t>;
template struct LaunchApproxConv2DOp<GPUDevice, float, float,
                                     NullApproxOpType_t>;
template struct LaunchApproxConv2DOp<GPUDevice, double, double,
                                     NullApproxOpType_t>;
} // namespace tensorflow
...
namespace tensorflow {
template struct LaunchApproxConv2DOp<GPUDevice, float, uint8,
                                     TableApproxOpType_t>;
}
