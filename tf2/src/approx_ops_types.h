...
EIGEN_ALWAYS_INLINE void Nudge(const float min, const float max,
 ...
 */
template <typename Device, typename T, typename AT> struct NullApproxOpType_t {
...
struct BaseTableApproxOpType_t {
  ...
  BaseTableApproxOpType_t(tensorflow::OpKernelConstruction *ctx)
      : bitWidth(8), tableFilename(""), quantMin(0), quantMax((1 << 8) - 1) {
    ...
    if (tableFilename.empty()) {
      VLOG(2) << "ApproxTF: Lookup table filename is empty... (using native "
  ...
  /**
   * @brief UpdateVarQuantProps updates quantization properties for single
   * tensor value given min/max of the tensor.
   ...
   */
  void UpdateVarQuantProps(float minValue, float maxValue,
                           VarQuantProps_t &quantProps) {
    ...
    if (minValue != 0.0f || maxValue != 0.0f) {
      tensorflow::Nudge(minValue, maxValue, quantMin, quantMax, &nudgedMin,
                        &nudgedMax, &nudgedScale);
  ...
  void Update(tensorflow::OpKernelContext *ctx) {
    {
      ...
      //            std::cout << "INPUT_RANGE: " << minValue << " " << maxValue
      //            << std::endl;
    }

    {
      ...
      //            std::cout << "FILTER_RANGE: " << minValue << " " << maxValue
      //            << std::endl;
    }
    ...
    //                  << ", input_scale: "     << quantProps.input.scale <<
    //                  std::endl
    //                  << ", input_inv_scale: " << quantProps.input.invScale <<
    //                  std::endl
    //                  << ", input_offset: "    << quantProps.input.offset <<
    //                  std::endl
    //                  << ", filter_scale: "     << quantProps.filter.scale <<
    //                  std::endl
    //                  << ", filter_inv_scale: " << quantProps.filter.invScale
    //                  << std::endl
    //                  << ", filter_offset: "    << quantProps.filter.offset <<
    //                  std::endl;
  }
   ...
   */
  tensorflow::Status LoadLookupTable(tensorflow::OpKernelConstruction *ctx,
                                     const std::string &filename) {
    std::fill(lookupTable.begin(), lookupTable.end(), ATVT(0));

    std::fstream file(filename.c_str(), std::ios::in | std::ios::binary);
    file.read(reinterpret_cast<char *>(lookupTable.data()),
              lookupTable.size() * sizeof(ATVT));

    VLOG(2) << "ApproxTF: Reading lookup table file: " << filename;
    TF_REQUIRES(file, tensorflow::errors::InvalidArgument(
                          "ApproxTF: Couldn't read the lookup table!"));
  ...
  std::string
      tableFilename; ///< Name of the binary file containing the lookup table.
...
struct TableApproxOpType_t
    : public BaseTableApproxOpType_t<T, AT, tensorflow::uint16> {
...
struct TableApproxOpType_t<Eigen::ThreadPoolDevice, T, AT>
    : public BaseTableApproxOpType_t<T, AT, tensorflow::uint16> {
  ...
  const tensorflow::uint16 *GetLookupData() const {
    return Base_t::lookupTable.data();
  }
};
 ...
 */
template <typename T, typename AT>
struct TableApproxOpType_t<Eigen::GpuDevice, T, AT>
    : public BaseTableApproxOpType_t<T, AT, tensorflow::uint16> {
  using Base_t = BaseTableApproxOpType_t<T, AT, tensorflow::uint16>;
